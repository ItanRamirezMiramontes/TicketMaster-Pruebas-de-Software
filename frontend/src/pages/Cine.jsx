...
import { Film, Popcorn, CreditCard, Calendar } from "lucide-react";
import api from "../api/axios";
import useForm from "../hooks/useForm";
import { useAuth } from "../context/AuthContext";

const ESTABLECIMIENTOS = ["CINEMEX", "CINEMARK", "OXFORD", "CINESA"];
const SERVICIOS = ["VIP", "IMAX", "4DX"];
const CLASIFICACIONES = ["AA", "A", "B", "B15", "C", "D"];
const METODOS = ["CREDITO", "DEBITO", "PAYPAL"];
const RESTRICCIONES = "Prohibido ingresar mascotas, armas y alimentos externos.";
  ...
  const handleSubmit = async (event) => {
    ...
    if (values.metodo !== "PAYPAL") {
      ...
      if (!/^[A-Za-z ]+$/.test(values.nombre_tarjeta.trim())) {
        setError("El nombre del titular solo puede contener letras.");
        return;
      }
    }
      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-900/90 p-8 shadow-xl shadow-slate-950/40">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-semibold text-slate-100">Compra entradas de cine</h2>
            <p className="mt-2 text-slate-400">Elige establecimiento, servicio y clasificación para tu función.</p>
          </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-6 rounded-3xl border border-slate-800 bg-slate-900/90 p-8">
          ...
          <div className="space-y-4 rounded-3xl border border-slate-800 bg-slate-950/80 p-6">
            <p className="text-sm uppercase tracking-[0.3em] text-indigo-400">Pago</p>
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="block text-sm text-slate-300">
                Método
                <select
        ...
        <aside className="space-y-6 rounded-3xl border border-slate-800 bg-slate-900/90 p-8">
          <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5">
            ...
            <p className="mt-4 text-slate-300">Establecimiento: {values.establecimiento}</p>
            <p className="text-slate-300">Servicio: {values.tipo_servicio}</p>
            <p className="text-slate-300">Clasificación: {values.clasificacion}</p>
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5">
            ...
            <p className="text-slate-300">{RESTRICCIONES}</p>
          </div>
          ...
          {reservation && (
            <div className="rounded-3xl border border-indigo-600 bg-slate-950/90 p-5">
              ...
              <p className="text-slate-300">Boletos: {values.boletos}</p>
              <p className="mt-3 text-slate-300">{reservation.detalles?.restricciones || RESTRICCIONES}</p>
            </div>
          )}
        </aside>
      </form>
