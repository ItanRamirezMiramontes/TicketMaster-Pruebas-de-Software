  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setEvents(response.data.map((event) => ({ ...event, type: "cine" })));
      } catch (err) {
        setError("Error al cargar la cartelera de cine.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <section className="space-y-8">
      <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950 shadow-2xl shadow-black/40">
        <img src={getImageUrl(events[0] || { images: [{ url: "https://placehold.co/1200x800?text=Cine" }] })} alt="Cartelera de cine" className="h-72 w-full object-cover md:h-[32rem]" />
        ...
        <div className="absolute inset-x-0 bottom-0 p-8 text-white">
          ...
          <p className="mt-3 max-w-2xl text-slate-300">{loading ? "Cargando películas..." : `${events.length} películas disponibles hoy`}</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-3xl border border-dashed border-white/10 bg-slate-900/80 p-12 text-center text-slate-400"><p className="text-2xl">No hay películas disponibles</p></div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{events.map((event) => (<EventCard key={event.id} event={event} type="cine" onBuyClick={openModal} />))}</div>
      )}
    </section>
  );
};
